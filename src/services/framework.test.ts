...
import { DbtModel, DbtProject } from '@shared/dbt/types';
...
describe('framework unit tests', () => {
  ...
  test('staging model sql and yml files are generated correctly', () => {
    ...
    expect(frameworkGenerateModelOutput({ modelJson, project }).sql).toBe(
      `
{{
  config(
...
`).trim(),
    ...
    expect(frameworkGenerateModelOutput({ modelJson, project }).yml).toBe(
      `version: "2"
models:
  - name: ${modelName}
...
select * from ${modelName}
  ...
  test('builds columns correctly', () => {
    ...
    const modelJson: FrameworkModel = {
      ...
      name: 'cur_aggragated',
      from: {
        model: 'stg__ibp__json__raw_cur_data',
      },
      select: [
        {
          name: 'count_star',
    ...
    expect(frameworkBuildColumns({ modelJson, project }).columns).toEqual([
      {
        name: 'count_star',
        data_type: 'number',
        meta: {
          type: 'fct',
          origin: {
            id: 'model.project.int__ibp__json__cur_aggragated',
          },
          agg: 'sum',
        },
      },
